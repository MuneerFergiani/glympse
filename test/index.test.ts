...
function addressToPublicKey(address) {
    ...
    const addressBuffer = ethereumUtil.toBuffer(address);
    const publicKeyBuffer = ethereumUtil.pubToAddress(addressBuffer);
  
    // The publicKeyBuffer now contains the corresponding public key.
    // You can convert it to a hexadecimal string if needed:
  ...
  describe('Glympse tests', () => {
    ...
    const getClaimInputs = async ({user} : { user: WalletClient }) => {
      ...
      return {
          ...
          nonce: claimer1.account!.address,
          sig: claimer1.account!.address,
      };
    };
    ...
    describe("Eligible user", () => {
      ...
      it.skip("Verifies a valid claim on-chain", async () => {
        ...
        await g.write.verifyResponse([toHex(proof.proof), toHex(unhashed_partial_decryption_key), toHex(responseValue), nullifier as `0x${string}`], { account: claimer1.account!.address });
